...
class PublicImageModelWrapper(ImageModelWrapper):
    ...
    def __init__(self, sess, model_fn_path, labels_path, image_shape,
        ...
        with graph.as_default():
            ...
            # ---this loss will be used to make gradient tensors
    ...
    def get_bottleneck_tensors(scope):
        ...
        graph = tf.compat.v1.get_default_graph()
        bn_endpoints = {}
        # ---go through all layers and find the concat layer
...
class KerasModelWrapper(ModelWrapper):
  ...
  """
    ...
    def __init__(
        ...
        self.labels = tf.io.gfile.GFile(labels_path).read().splitlines()
        # for 3Dcnn input, it also need depth information, defalut[32,32,3,8]
        # self.image_shape = [int(imageshape[0]), int(imageshape[1]), 3, int(imageshape[2])]
        ...
        #         labels=tf.one_hot(
        #             self.y_input,
        #             self.model.outputs[0].get_shape().as_list()[1]),
        #         logits=self.pred))
